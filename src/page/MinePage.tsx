...
export default function MinePage({ imageAddedKey }: MinePageProps) {
  ...
  const fetchImages = async () => {
    setLoading(true)
    try {
      const res = await axiosInstance.get<ImageData[]>('/images/mine')
      setImages(res.data)
    } catch (err) {
  ...
  const handleDeleted = (deletedId: string) => {
    setImages((imgs) => imgs.filter((i) => i.id !== deletedId))
  }
        ...
        {loading && images.length === 0 ? (
          ...
          <div className="mt-8 flex justify-center">
            <img
              ...
              src="https://media1.tenor.com/m/WnjJsVOwoJQAAAAC/john-travolta-well.gif"
              alt="Aucun résultat"
            />
          </div>
        ) : (
